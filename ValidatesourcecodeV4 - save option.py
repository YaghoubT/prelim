...
def EditSourceCode(SourceCode):
    ...
    while Choice != "C":
        ...
        while Choice != "E" and Choice != "C" and Choice != "S":
            ...
            Choice = input("Enter your choice: ")
        if Choice == "E":
          NewLineEdit(SourceCode,LineNumber)   
        elif Choice == "S":
...
def NewLineEdit(SourceCode, LineNumber):
  ...
  NewComment = input("Input a new comment (input nothing if no comment): ")
  if NewComment != EMPTY_STRING:
    if "*" not in NewComment:
      NewComment = "* " +  NewComment
  while len(NewLabel) != 6:
    NewLabel = " " + NewLabel
  while len(NewOpcode) != 5:
    NewOpcode = NewOpcode + " "
  while len(NewOperand) != 5:
    NewOperand = NewOperand + " "
    Line = NewLabel + " " + NewOpcode + NewOperand + "   " + NewComment
