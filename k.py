...
def DisplayCurrentState(SourceCode, Memory, Registers):
  ...
  print("*  Status Register: ZNVCP")
  print("*                  ", ConvertToBinary(Registers[STATUS]))
  DisplayFrameDelimiter(-1)
  
def SetFlags(Value, Registers):
  if Value == 0:
    Registers[STATUS] = ConvertToDecimal("10000")
  elif Value < 0:
    Registers[STATUS] = ConvertToDecimal("01000")
  elif Value > MAX_INT or Value < -(MAX_INT + 1):
    Registers[STATUS] = ConvertToDecimal("00110") #carry flag triggered when V flag triggered
  else:
    Registers[STATUS] = ConvertToDecimal("00000")
    Number = ConvertToBinary(Registers[ACC]) #during CMP the value in the ACC is not always assigned to the variable Value
    if Number.count("1") % 2 == 0:
      Registers[STATUS] = ConvertToDecimal(Number + "1")
  return Registers
...
def ExecuteAND(Registers, Binary1, Binary2):
  x = 0
  Result = ""
  while x < len(Binary1) - 1:
    if Binary1[x] and Binary2[x] == "1":
      Result += "1"
    else:
      Result += "0"
    x += 1
...
def ExecuteORR(Registers, Binary1, Binary2):
  x = 0
  Result = ""
  while x < len(Binary1) - 1:
    if Binary1[x] or Binary2[x] == "1":
      Result += "1"
    else:
      Result += "0"
    x += 1
...
def ExecuteEOR(Registers, Binary1, Binary2):
  ...
  Result = ""
  while x < len(Binary1) - 1:
    if (Binary1[x] == "1" and Binary2[x] == "0") or (Binary2[x] == "1" and Binary1[x] == "0"):
      Result += "1"
    else:
      Result += "0"
    x += 1
...
def ExecuteMVN(Registers, Binary):
  x = 0
  Result = ""
...
def Execute(SourceCode, Memory): 
  ...
  while OpCode != "HLT":
    line = Registers[TOS] - 1
    if Memory[line].OpCode != EMPTY_STRING:
